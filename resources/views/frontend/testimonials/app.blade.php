@extends("frontend.v_layout.app")

@section("title", "Testimonials | Furnish")

@section("content")
    <main>
        <section class="py-lg-8 py-5 text-center">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-6">
                        <h1 class="display-5 mb-3">What Our Customers Say</h1>
                        <p class="text-muted mb-0 lead">We value each voice and every experience. Here's what people say
                            about Furnish.</p>
                    </div>

                </div>
            </div>
        </section>
        <section class="pb-8">
            <div class="container ">
                <div class="row g-4">
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic ">
                                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor
                                        incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud
                                        exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">John Deo</h4>
                                        <small>RMO, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic">
                                        "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                                        ex ea commodo consequat. cupidatat non proident, sunt in culpa qui officia deserunt
                                        mollit anim id est laborum."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">Ria Dong</h4>
                                        <small>CEO, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic">
                                        "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                                        ex ea commodo consequat. cupidatat non proident, sunt in culpa qui officia deserunt
                                        mollit anim id est laborum."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">Kenneth Gumbs</h4>
                                        <small>Salesperson, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic">
                                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor
                                        incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud
                                        exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">John Deo</h4>
                                        <small>RMO, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic">
                                        "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                                        ex ea commodo consequat. cupidatat non proident, sunt in culpa qui officia deserunt
                                        mollit anim id est laborum."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">Joseph Perez</h4>
                                        <small>Quality inspector, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-lg-4 col-12">
                        <div class="card  ">
                            <div class="card-body p-6">
                                <div class="text-center">
                                    <p class="fst-italic">
                                        "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip
                                        ex ea commodo consequat. cupidatat non proident, sunt in culpa qui officia deserunt
                                        mollit anim id est laborum."
                                    </p>
                                    <div class="lh-1">
                                        <h4 class="fs-5 mb-1">Christopher Caruso</h4>
                                        <small>Deputy, Company Name</small>
                                    </div>
                                </div>


                            </div>

                        </div>
                    </div>
                    <div class="col-12">
                        <div class="text-center">
                            <a href="#" class="btn btn-outline-dark"> Load More</a>

                        </div>

                    </div>

                </div>
            </div>
        </section>
    </main>
@endsection